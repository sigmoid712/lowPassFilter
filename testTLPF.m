...
n = 10;
for i=1:20
F1 = TLPF(I1,i*n/2,i*n);
figure(i);
set(figure(i),'visible','off','Name',['梯形低通滤波器D0=',num2str(i*n)]);
subplot(2,3,1);
imshow(I0);
title('原图');
subplot(2,3,4);
...
title('滤波后频谱');
saveas(gcf,['result/TLPF/GaussianNoise/梯形低通滤波器Do=',num2str(i*n),'.jpg'])
end

n = 10;
for i=1:20
F2 = TLPF(I2,i*n/2,i*n);
figure(i);
set(figure(i),'visible','off','Name',['梯形低通滤波器D0=',num2str(i*n)]);
subplot(2,3,1);
imshow(I0);
...
title('滤波后频谱');
saveas(gcf,['result/TLPF/SaltNoise/梯形低通滤波器Do=',num2str(i*n),'.jpg'])
end

n = 10;
for i=1:20
F3 = TLPF(I3,i*n/2,i*n);
figure(i);
set(figure(i),'visible','off','Name',['梯形低通滤波器D0=',num2str(i*n)]);
subplot(2,3,1);
imshow(I0);
...
title('滤波后频谱');
saveas(gcf,['result/TLPF/SpeckleNoise/梯形低通滤波器Do=',num2str(i*n),'.jpg'])
end

n = 10;
for i=1:20
F4 = TLPF(I4,i*n/2,i*n);
figure(i);
set(figure(i),'visible','off','Name',['梯形低通滤波器D0=',num2str(i*n)]);
subplot(2,3,1);
imshow(I0);
...
title('滤波后频谱');
saveas(gcf,['result/TLPF/PoissonNoise/梯形低通滤波器Do=',num2str(i*n),'.jpg'])
end