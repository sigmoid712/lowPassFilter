...
F1 = BLPF(I1,8,i*n);
figure(i);
set(figure(i),'visible','off','Name',['Butterworth低通滤波器D0=',num2str(i*n)]);
subplot(2,3,1);
imshow(I0);
title('原图');
subplot(2,3,4);
...
title('滤波后频谱');
saveas(gcf,['result/BLPF/GaussianNoise/Butterworth低通滤波器Do=',num2str(i*n),'.jpg'])
end
...
title('滤波后频谱');
saveas(gcf,['result/BLPF/SaltNoise/Butterworth低通滤波器Do=',num2str(i*n),'.jpg'])
end
...
title('滤波后频谱');
saveas(gcf,['result/BLPF/SpeckleNoise/Butterworth低通滤波器Do=',num2str(i*n),'.jpg'])
end
...
title('滤波后频谱');
saveas(gcf,['result/BLPF/PoissonNoise/Butterworth低通滤波器Do=',num2str(i*n),'.jpg'])
end